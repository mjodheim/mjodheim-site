...
export default function ChroniquesClient({ articles }: { articles: Article[] }) {
  return (
    <>
      {/* Hero */}
      <section
          ...
          {articles.map((article, i) => (
            ...
            >
              <div style={{ position: "relative", minHeight: "250px" }}>
                <Image src={`/images/${article.image}`} alt={article.title} fill style={{ objectFit: "cover" }} />
              </div>
              <div style={{ padding: "2.5rem 2.5rem 2.5rem 0", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: "0.75rem", color: "rgba(245,230,204,0.4)" }}>
                    {article.date}{article.readTime ? ` · ${article.readTime} de lecture` : ""}
                  </span>
                </div>
                <h2 style={{ fontFamily: "var(--font-cormorant), Georgia, serif", fontSize: "1.8rem", fontWeight: 500, color: "#F5E6CC", marginBottom: "1rem", lineHeight: 1.25 }}>
                  {article.title}
                </h2>
                ...
                <Link
                  ...
                  style={{ color: "#C9A84C", fontSize: "0.85rem", letterSpacing: "0.1em", textTransform: "uppercase", textDecoration: "none", display: "flex", alignItems: "center", gap: "0.5rem", fontFamily: "var(--font-cormorant), Georgia, serif", fontWeight: 500 }}
                >
                  Lire la chronique →
                </Link>
              </div>
